...
window.initTripExplorer=function(){
 ...
 function renderNearby(key){nearby.innerHTML='<h3>시간 남으면, 이 근처</h3><p>예전에 저장해 둔 곳 중 오늘 동선과 가까운 후보만 모았습니다. 꼭 갈 곳은 아니니, 출발 전 영업시간과 대기 여부만 확인하세요.</p>';
 (data.nearby&&data.nearby[key]||[]).forEach(function(g){var d=document.createElement('details');d.className='nearby-group';d.innerHTML='<summary>'+escapeText(g.title)+'<small>'+(g.items.length?g.items.length+'곳 · 펼치기':'돌아갈 시간 확인')+'</small></summary><div class="nearby-content">'+(g.image?'<img class="nearby-banner" loading="lazy" src="'+escapeText(g.image)+'" alt="'+escapeText(g.caption)+'"><small class="nearby-caption">'+escapeText(g.caption)+' · 동네 참고 사진</small>':'')+'<p class="nearby-intro">'+escapeText(g.note)+'</p><div class="nearby-items">'+g.items.map(function(p){return '<article class="nearby-place"><h4>'+escapeText(p.name)+'</h4><p>'+escapeText(p.note)+'</p><a target="_blank" rel="noreferrer" href="https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(p.query)+'">위치·영업 확인 ↗</a></article>'}).join('')+'</div></div>';nearby.appendChild(d)})}
 ...
 function renderDay(key){
  ...
  day.places.forEach(function(p,i){
   ...
   card.innerHTML='<div class="trip-card-main">'+media+'<button class="trip-select" type="button" aria-pressed="false"><div class="trip-kicker">'+number+' / '+escapeText(p[3])+'</div><h3>'+escapeText(p[0])+'</h3><p>'+escapeText(note||'위치는 지도에서 확인.')+'</p></button></div><div class="trip-card-actions"><button class="trip-locate" type="button">위치 보기 ↗</button><a class="trip-go" target="_blank" rel="noreferrer" href="https://www.google.com/maps/search/'+encodeURIComponent(p[0])+'/@'+p[1]+','+p[2]+',18z">Google 지도 열기 ↗</a></div>';
   ...
   if(photo){card.querySelector('.trip-photo').addEventListener('click',function(){showPhoto(p[0],photo)});card.querySelector('img').addEventListener('error',function(){var box=this.parentElement;this.remove();box.classList.add('trip-no-photo');box.innerHTML='<span>'+number+'</span><em>사진을 불러오지 못함</em>';box.disabled=true},{once:true})}
   list.appendChild(card);
  });
  if(!visibleIndices.length)list.innerHTML='<p class="trip-empty">이 날짜에는 이 분류로 저장한 곳이 없습니다.</p>';
  caption.innerHTML='<b>'+escapeText(day.label)+' · '+escapeText(day.short)+'</b><p>'+visibleIndices.length+'곳 · 점선은 저장된 순서를 이어 본 참고선입니다.<br>실제 이동은 Google 지도에서 확인하세요.</p>';
...
var warning=document.createElement('div');warning.className='trip-map-failure';warning.hidden=true;warning.innerHTML='<b>배경 지도를 불러오지 못했습니다.</b><p>저장해 둔 전체 지도는 아래 링크에서 볼 수 있습니다.</p><a href="'+escapeText(data.mapUrl)+'" target="_blank" rel="noreferrer">Google 지도 열기 ↗</a>';node.parentElement.appendChild(warning);var tileErrors=0;if(tiles.on)tiles.on('tileerror',function(){if(++tileErrors>=2)warning.hidden=false});
layer=L.layerGroup().addTo(map);line=L.layerGroup().addTo(map);renderMap()}
 renderDay(keys[0]);tryMap();
 if(!map){node.innerHTML='<p class="trip-empty">지도를 불러오는 중입니다. 계속 보이지 않으면 장소 카드에서 Google 지도를 열어 주세요.</p>';var tries=0,timer=setInterval(function(){if(window.L){node.innerHTML='';tryMap();clearInterval(timer)}else if(++tries>=20)clearInterval(timer)},500)}
 return true;
};
